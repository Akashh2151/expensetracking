...
def limitn():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute('SELECT limitss FROM limits WHERE userid = % s ORDER BY id DESC LIMIT 1', (str(session['id']), ))
    x = cursor.fetchone()
    if x is not None:
      s = x[0]
      
    else:
    # Provide a default value or handle the case where no limit is found for the user.
      s = 0 
